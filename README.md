...
something cool to have, having been quite fond CS:GO players rarely queuing as a 5-player team.
...
people with similar ability or, being really visionary and optimistic about the app's chances of great success, matching
people who are geographically close and having them connect to a server in their region.
...
mitigated by the use of lightweight green threads in place of fully fledged OS threads, but we really don't know whether
the next solution I'm going to propose _would have been_/_would be_ better.
...
short-term, if the app sees any kind of significant usage, which *strangely* feels like it's not very likely D:), which
isn't a worry with the current architecture and implementation, as each request's side-effects are managed when they
could start having an effect and not when they actually might have an effect, if this makes any sense.

If that sentence or this whole section (maybe this entire document) *doesn't*, maybe this next bit will, as it goes more
specifically into the details of each of the two cases in which timing arises as an issue in our app.
